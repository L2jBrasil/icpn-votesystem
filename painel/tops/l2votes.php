...
if(@fsockopen(str_replace("https://","",str_replace("http://","",$row->top_url)), 80 , $errno , $errstr , 30)){
	if(isset($_COOKIE["l2votes"])){
		$data_modificada = pega_cookie($_COOKIE["l2votes"]);
	}else{
	...
	if(strtotime($data_modificada) >= strtotime(date('Y-m-d H:i:s'))){
		...
		?>
		<div style='width:87px; height:47px; border:1px solid #999; margin-top:5px; margin-left:5px; float:left;'><a href='https://l2votes.com/votes.php?sid=<?php echo $row->top_id; ?>' target='_blank'><img src='images/buttons/<?php echo $row->top_img; ?>' title='l2votes.com' border='0' width='87' height='47' onClick="javascript:SetCookie('l2votes');"></a></div>
		<?php
	}
}else{
