...
if(@fsockopen(str_replace("https://","",str_replace("http://","",$row->top_url)), 80 , $errno , $errstr , 30)){
	...
	if(strtotime($data_modificada) >= strtotime(date('Y-m-d H:i:s'))){
		...
		?>
		<div style='width:87px; height:47px; border:1px solid #999; margin-top:5px; margin-left:5px; float:left;'><a href='https://www.gamebytes.net/index.php?a=in&u=<?php echo $row->top_id; ?>' target='_blank'><img src='images/buttons/<?php echo $row->top_img; ?>' title='GameBytes - Best Lineage 2 Toplist' border='0' width='87' height='47' onClick="javascript:SetCookie('gamebytes');"></a></div>
		<?php
	}
}else{
