...
if(@fsockopen(str_replace("https://","",str_replace("http://","",$row->top_url)), 80 , $errno , $errstr , 30)){
	...
	$pagina = @file_get_contents("https://private-server.ws/index.php?a=in&u=".$row->top_id."&ipc=".get_client_ip());
	if($pagina[0] == 1 && !isset($_COOKIE["privateserverws"])){
		?>
		<script language="javascript">
			SetCookie('privateserverws');
		</script>
		<?php
	}
	if(isset($_COOKIE["privateserverws"])){
		$data_modificada = pega_cookie($_COOKIE["privateserverws"]);
	}else{
	...
	if(strtotime($data_modificada) >= strtotime(date('Y-m-d H:i:s'))){
		...
		<div style='width:87px; height:47px; border:1px solid #999; margin-top:5px; margin-left:5px; float:left;'>
			<a href='https://private-server.ws/index.php?a=in&u=<?php echo $row->top_id; ?>' target='_blank'><img src='images/buttons/<?php echo $row->top_img; ?>' title='MMORPGs Top 100 Gaming list for 2017' border='0' width='87' height='47' onClick="javascript:SetCookie('privateserverws');"></a>
		</div>
		<?php
	}
}else{
