...
if(@fsockopen(str_replace("https://","",str_replace("http://","",$row->top_url)), 80 , $errno , $errstr , 30)){
	...
	$pagina = @file_get_contents("https://api.hopzone.net/lineage2/vote?token=".$row->top_token."&ip_address=".get_client_ip());
	...
	if(strtotime($data_modificada) >= strtotime(date('Y-m-d H:i:s'))){
		...
		<div style='width:87px; height:47px; border:1px solid #999; margin-top:5px; margin-left:5px; float:left;'>
			<a href='https://vgw.hopzone.net/site/vote/<?php echo $row->top_id; ?>/1' target='_blank'><img src='images/buttons/<?php echo $row->top_img; ?>' title='Vote our server on HopZone.Net' border='0' width='87' height='47'"></a>
		</div>
		<?php
	}
}else{
