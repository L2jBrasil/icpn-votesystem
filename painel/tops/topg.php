...
if(@fsockopen(str_replace("https://","",str_replace("http://","",$row->top_url)), 80 , $errno , $errstr , 30)){
	if(isset($_COOKIE["topg"])){
		$data_modificada = pega_cookie($_COOKIE["topg"]);
	}else{
	...
	if(strtotime($data_modificada) >= strtotime(date('Y-m-d H:i:s'))){
		...
		?>
		<div style='width:87px; height:47px; border:1px solid #999; margin-top:5px; margin-left:5px; float:left;'><a href='https://topg.org/lineage2-private-servers/in-<?php echo $row->top_id; ?>' target='_blank' onClick="javascript:SetCookie('topg');"><img src='images/buttons/<?php echo $row->top_img; ?>' title='lineage2 private servers' border='0' width='87' height='47'></a></div>
		<?php
	}
}else{
