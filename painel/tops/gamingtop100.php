...
if(@fsockopen(str_replace("https://","",str_replace("http://","",$row->top_url)), 80 , $errno , $errstr , 30)){
	...
	$pagina = @file_get_contents("http://www.gamingtop100.net/ip_check/".$row->top_id."/".get_client_ip());
	if($pagina[0] == 1 && !isset($_COOKIE["gamingtop100"])){
		?>
		<script language="javascript">
			SetCookie('gamingtop100');
		</script>
		<?php
	}
	if(isset($_COOKIE["gamingtop100"])){
		$data_modificada = pega_cookie($_COOKIE["gamingtop100"]);
	}else{
	...
	if(strtotime($data_modificada) >= strtotime(date('Y-m-d H:i:s'))){
		...
		<div style='width:87px; height:47px; border:1px solid #999; margin-top:5px; margin-left:5px; float:left;'>
			<a href='http://www.gamingtop100.net/in-<?php echo $row->top_id; ?>' target='_blank'><img src='images/buttons/<?php echo $row->top_img; ?>' title='lineage 2 private servers' border='0' width='87' height='47' onClick="javascript:SetCookie('gamingtop100');"></a>
		</div>
		<?php
	}
}else{
