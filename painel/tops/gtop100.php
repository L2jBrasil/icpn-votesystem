...
if(@fsockopen(str_replace("https://","",str_replace("http://","",$row->top_url)), 80 , $errno , $errstr , 30)){
	if(isset($_COOKIE["gtop100"])){
		$data_modificada = pega_cookie($_COOKIE["gtop100"]);
	}else{
	...
	if(strtotime($data_modificada) >= strtotime(date('Y-m-d H:i:s'))){
		...
		?>
		<div style='width:87px; height:47px; border:1px solid #999; margin-top:5px; margin-left:5px; float:left;'><a href='https://gtop100.com/topsites/Lineage-2/sitedetails/<?php echo $row->top_id; ?>?vote=1' target='_blank'><img src='images/buttons/<?php echo $row->top_img; ?>' title='Vote no L2 Top 200' border='0' width='87' height='47' onClick="javascript:SetCookie('gtop100');"></a></div>
		<?php
	}
}else{
