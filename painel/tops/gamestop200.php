...
if(@fsockopen(str_replace("https://","",str_replace("http://","",$row->top_url)), 80 , $errno , $errstr , 30)){
	...
	$pagina = @file_get_contents("https://api.gamestop200.com/check/".$row->top_id."/".get_client_ip());
	$pagina = json_decode($pagina);
	$data_modificada = !empty($pagina->date) ? date("Y-m-d H:i:s",strtotime($pagina->date." + 12 hours")) : '0000-00-00 00:00:00';
	if(strtotime($data_modificada) >= strtotime(date('Y-m-d H:i:s'))){
		...
		<div style='width:87px; height:47px; border:1px solid #999; margin-top:5px; margin-left:5px; float:left;'>
			<a href='https://www.gamestop200.com/details/<?php echo $row->top_id; ?>/vote' target='_blank'><img src='images/buttons/<?php echo $row->top_img; ?>' title='Extreme Game Sites' border='0' width='87' height='47'"></a>
		</div>
		<?php
	}
}else{
