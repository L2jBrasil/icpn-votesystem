...
if(@fsockopen(str_replace("https://","",str_replace("http://","",$row->top_url)), 80 , $errno , $errstr , 30)){
	...
	$pagina = @file_get_contents("https://l2top.co/reward/VoteCheck.php?id=".$row->top_id."&ip=".get_client_ip());
	if(trim($pagina) == "TRUE" && !isset($_COOKIE["l2top.co"])){
		?>
	...
	if(isset($_COOKIE["l2top.co"])){
		$data_modificada = pega_cookie($_COOKIE["l2top.co"]);
	}else{
	...
	if(strtotime($data_modificada) >= strtotime(date('Y-m-d H:i:s'))){
		...
		<div style='width:87px; height:47px; border:1px solid #999; margin-top:5px; margin-left:5px; float:left;'>
			<a href='http://l2top.co/vote/server/<?php echo $row->top_id; ?>' target='_blank'><img src='images/buttons/<?php echo $row->top_img; ?>' title='Vote in L2Top.CO - Lineage 2' border='0' width='87' height='47' onClick="javascript:SetCookie('l2top.co');"></a>
		</div>
		<?php
	}
}else{
